...
\title[]{Potential Research Collaborations with Bayer}
...
\begin{frame}{Data-driven and modular controller design}
...
\begin{frame}{Topics for potential collaboration}
	\begin{enumerate}
		\pause
		\item Parameter estimation for plant models
		\pause
		\item Model reduction strategies
		\pause
		\item Simple RL approaches with domain knowledge
		\pause
		\item Combine RL and MPC
		\pause
		\item Anything involving MPC!
	\end{enumerate}
\end{frame}

\begin{frame}{Parameter estimation}
	\begin{align*}
		\min_{\theta} \ & \sum_{k=0}^{N-1} |y(k)-\hat{y}(k)|^2 + \ell(\theta) \\
		\textnormal{s.t. } & \dot{x} = f(x,\theta) \\
		& y(k) = h(x(k))
	\end{align*}
	\begin{itemize}
		\pause
		\item Regularizer $\ell(\cdot)$ based on prior information about 
		$\theta$.
		\pause
		\item Implicit Euler to discretize $\dot{x} = f(x,\theta)$.
		\pause
		\item Solve as nonlinear program (NLP).
		\pause
		\item Examples for biological systems with 2,352
		parameters, 2,304 differential equations, and 20,352 data 
		points. \pause Solved in 15 minutes.\cite{shin:venturelli:zavala:2019}
	\end{itemize}
\end{frame}

\begin{frame}{$+$ Model reduction}
	\begin{itemize}
		\item Can solve enforce sparse solutions with $\ell(\theta)$.
		\item Can quantify uncertainty: $\theta \sim 
		\mathcal{N}(\bar{\theta},\Sigma_{\theta})$.
		\item Can check observability/uniqueness of parameters.
	\end{itemize}
	\begin{block}{}
		\centering
		All of this information can be used to inform model reduction 
		strategies!
	\end{block}
\end{frame}
...
\begin{frame}[noframenumbering]{Applications to CEA: Models}
	\onslide<2->{
	\begin{equation*}
...
\begin{frame}[noframenumbering]{Optimal control}
	\begin{tikzpicture}[circle dotted/.style={dash pattern=on .05mm off 6mm,
			line cap=round}]
		\begin{axis}
			[
...
\begin{frame}[noframenumbering]{MPC and RL}
	\begin{center}
...
\begin{frame}[noframenumbering]{A natural hierarchy for plants}
	\centering
	\begin{columns}
